...
fn parseBootState() ?BootState {
    ...
    return .{
        ...
        .root_token = readCfgU64(block_bootstrap.root_token_index),
...
fn waitForBootResources() void {
    while (true) {
        ...
        if (boot_state.endpoint_id != 0 and
            fs_abi.isCapToken(boot_state.root_token) and
...
fn handleConnectRequest(request_paddr: u64) void {
    for (&sessions, 0..) |*session, slot| {
        ...
        if (mapPage(req_va, request_paddr, false) != syscall_ok) return;
        ...
        if (mapPage(resp_va, request.arg0, true) != syscall_ok) return;
        ...
        const child_token = grantFsCap(boot_state.root_token, request.arg1, rights);
        if (!fs_abi.isCapToken(child_token)) {
            _ = userLog("VirtioBlk: grant child block cap failed\n");
            return;
        }
        session.* = .{
            .active = true,
...
pub export fn _start() noreturn {
    ...
    while (true) {
        ...
        if (received >= 0x1000 and !fs_abi.isCapToken(received)) {
