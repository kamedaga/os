...
fn isFsCapToken(token: u64) bool {
    return (token & fs_cap_token_tag) != 0 and (token & ~fs_cap_token_tag) != 0;
}
...
fn rootNode(state: *const BootState) ResolvedNode {
    return .{
        .source_mount_token = state.root_mount_token,
...
fn nodeFromBootFsChild(child: BootFsChild) ResolvedNode {
    return .{
        .source_mount_token = boot_state.root_mount_token,
...
fn runBootSelfTest(state: *const BootState) bool {
    ...
    if (root.source_mount_token != state.root_mount_token or root.kind != .dir) {
...
fn grantNodeToken(session: *Session, node: ResolvedNode) ?u64 {
    ...
    const server_token = installFsCap(node.object_id, object_kind, nodeServerRights(node));
    if (!fs_abi.isCapToken(server_token)) return null;
    const client_token = grantFsCap(server_token, session.client_process_slot, nodeClientRights(node));
...
fn grantOpenFileToken(session: *Session, node: ResolvedNode) ?u64 {
    const server_token = installFsCap(openFileObjectId(node), .open_file, openFileServerRights());
    if (!fs_abi.isCapToken(server_token)) return null;
    const client_token = grantFsCap(server_token, session.client_process_slot, openFileClientRights());
    if (!fs_abi.isCapToken(client_token)) return null;
