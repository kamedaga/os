...
fn requireProcessSlot() u64 {
    const slot = getProcessSlot();
    if (slot != 0) return slot;
    _ = userLog("Init: process slot unavailable\n");
...
fn grantBlockDriverResources(config_source_va: u64, descriptor: init_bootstrap_abi.BlockDeviceDescriptor, child_process_slot: u64) bool {
    const page_right_cpu_read: u64 = 0x1;
    const page_right_cpu_write: u64 = 0x2;
    const common_paddr = descriptor.common_page_paddr;
    const notify_paddr = descriptor.notify_page_paddr;
...
fn grantBootstrapFsBlockResources(config_source_va: u64, descriptor: init_bootstrap_abi.BlockDeviceDescriptor, child_process_slot: u64) bool {
    const page_right_cpu_read: u64 = 0x1;
    const page_right_cpu_write: u64 = 0x2;
    const common_paddr = descriptor.common_page_paddr;
    const notify_paddr = descriptor.notify_page_paddr;
...
const LaunchContext = struct {
    ...
    fn findCachedExec(self: *LaunchContext, path: []const u8) ?fs_client.OpenResult {
    ...
    fn storeCachedExec(self: *LaunchContext, path: []const u8, result: fs_client.OpenResult) void {
    ...
    fn fetchExecForStartupPath(self: *LaunchContext, path: []const u8, label: []const u8) fs_client.OpenResult {
        if (self.bootstrap_fs_process_slot != null and self.bootstrap_fs_endpoint_id != null) {
            return self.requireExecFromBootstrapFs(path, label) orelse {
                _ = userLog("Init: bootstrap fs exec failed\n");
                while (true) asm volatile ("pause");
            };
        }
        return self.requireExecFromBootFs(path, label);
    }

    fn cacheExecForStartupPath(self: *LaunchContext, path: []const u8, label: []const u8) void {
        if (self.findCachedExec(path) != null) return;
        const exec = self.fetchExecForStartupPath(path, label);
        self.storeCachedExec(path, exec);
    }

    fn requireExecForStartupPath(self: *LaunchContext, path: []const u8, label: []const u8) fs_client.OpenResult {
        if (self.findCachedExec(path)) |exec| return exec;
        return self.fetchExecForStartupPath(path, label);
    }
    ...
    fn launchVfsForSpec(self: *LaunchContext, spec: StartupRoleSpec, path: []const u8) void {
        const exec = self.requireExecForStartupPath(path, spec.label);
    ...
    fn launchBootstrapFsForSpec(self: *LaunchContext, spec: StartupRoleSpec, path: []const u8) void {
        ...
        if (!grantBootstrapFsBlockResources(config_source_va, block_desc, child_slot)) {
            self.logRoleLine("grant", spec.label, "block resources failed");
            while (true) asm volatile ("pause");
        }
        _ = signalEndpoint(endpoint_id);
    ...
    fn armCompositor(self: *LaunchContext, classic_path: ?[]const u8, gpu_path: ?[]const u8) void {
        ...
        const compositor_exec = self.requireExecForStartupPath(classic, "compositor");
        const gpu_compositor_exec = self.requireExecForStartupPath(gpu, "gpu compositor");
    ...
    fn launchRole(self: *LaunchContext, role: startup_plan_abi.StartupProgramRole, path: []const u8) void {
        ...
        switch (spec.action) {
            ...
            .bootstrap_fs_server => self.launchBootstrapFsForSpec(spec, path),
...
fn runStartupManifest(ctx: *LaunchContext, manifest: *const StartupManifest) void {
    ...
    while (lines.next()) |raw_line| {
        ...
        switch (spec.action) {
            .bootstrap_fs_server => ctx.launchRole(parsed.role, parsed.path),
            .input_driver => ctx.launchRole(parsed.role, parsed.path),
            .deferred_compositor => {
                ctx.cacheExecForStartupPath(parsed.path, spec.label);
