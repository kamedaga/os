...
int main(void) {
    ...
    if (expect(code[7] == LPR_ZPOLINE_PATCH_TO0 && code[8] == LPR_ZPOLINE_PATCH_TO1)) return 1;
    if (expect(code[10] == LPR_ZPOLINE_PATCH_TO0 && code[11] == LPR_ZPOLINE_PATCH_TO1)) return 1;
    ...
    if (expect(LPR_ZPOLINE_DIRECT_LIMIT > 1024)) return 1;
