...
static int64_t lpr_linux_clone_frame_impl(const struct lpr_linux_user_frame *user_frame, uint64_t flags, uint64_t child_stack, uint64_t parent_tid, uint64_t child_tid, uint64_t tls)
{
    ...
    filed_exec_path_t fork_snapshot;
    lpr_memset(&fork_snapshot, 0, sizeof(fork_snapshot));
    lpr_exec_transaction_t fork_transaction;
...
int64_t lpr_linux_execve(uint64_t path_raw, uint64_t argv_raw, uint64_t envp_raw)
{
    ...
    exec.flags = FILED_EXEC_LINUX_LPR | FILED_EXEC_SELF;
    ...
    status = lpr_prepare_exec_manifest(&exec, &transaction);
    if (status != 0) {
        lpr_destroy_exec_transaction(&transaction);
        return status;
    }
