...
static int termd_send_reply_with_error(
    int reply_fd,
    void *page,
    const pacha_service_request_header_t *request_header,
    uint64_t request_id,
    int64_t status,
    uint64_t result,
...
static int termd_send_tty_signal_to_supervisor(termd_service_t *service, uint32_t pgrp, uint32_t signo)
{
    ...
    header->op = LPRS_V2_OP_SIGNAL_DELIVER_TTY;
    header->flags = PACHA_SERVICE_FLAG_PAGE_PAYLOAD;
    header->request_id = request_id;
    header->trace_id = request_id;
    header->payload_size = sizeof(lprs_v2_tty_signal_t);
    lprs_v2_tty_signal_t *payload =
        (lprs_v2_tty_signal_t *)((uint8_t *)page + PACHA_SERVICE_HEADER_BYTES);
    ...
    *out_result = 0;
    ...
    case TERMD_V2_OP_OPEN_PTMX:
        ...
        return termd_linux_tty_island_open_ptmx(
            tty,
            ((const termd_v2_open_request_t *)payload)->flags,
            out_result);
    case TERMD_V2_OP_OPEN_PTS:
        if (payload == 0) {
            return TERMD_ERR_INVAL;
        }
        return termd_linux_tty_island_open_pts(
            tty,
            (const termd_v2_open_request_t *)payload,
            out_result);
    case TERMD_V2_OP_OPEN_HVC:
        if (payload == 0) {
            return TERMD_ERR_INVAL;
        }
    ...
    case TERMD_V2_OP_OPEN_CTTY:
        ...
        return termd_linux_tty_island_open_ctty(
            ...
            (const termd_v2_open_request_t *)payload,
            out_result);
    case TERMD_V2_OP_HANDLE_CLOSE:
        if (payload == 0) {
            return TERMD_ERR_INVAL;
        }
        return termd_linux_tty_island_close(
            tty,
            ((const termd_v2_handle_request_t *)payload)->handle);
    case TERMD_V2_OP_HANDLE_DUP:
        if (payload == 0) {
            return TERMD_ERR_INVAL;
        }
        return termd_linux_tty_island_dup(
            ...
            ((const termd_v2_handle_request_t *)payload)->handle,
            out_result);
    case TERMD_V2_OP_SIGNAL_TAKE:
        if (payload == 0) {
            return TERMD_ERR_INVAL;
        }
        return termd_linux_tty_island_take_signal(
            tty,
            (termd_v2_signal_request_t *)payload,
            out_result);
    case TERMD_V2_OP_HANDLE_READ:
    case TERMD_V2_OP_HANDLE_WRITE:
        if (payload == 0) {
            return TERMD_ERR_INVAL;
        }
        return termd_linux_tty_island_io(
            tty,
            op == TERMD_V2_OP_HANDLE_WRITE,
            (termd_v2_io_request_t *)payload,
            out_result);
    case TERMD_V2_OP_HANDLE_POLL:
        if (payload == 0) {
            return TERMD_ERR_INVAL;
        }
        return termd_linux_tty_island_poll(tty, (termd_v2_poll_request_t *)payload);
    case TERMD_V2_OP_HANDLE_IOCTL:
        if (payload == 0) {
            return TERMD_ERR_INVAL;
        }
        return termd_linux_tty_island_ioctl(tty, (termd_v2_ioctl_request_t *)payload);
    default:
        return TERMD_ERR_INVAL;
    }
}
...
void termd_service_forward_pending_tty_signals(termd_service_t *service)
{
    ...
    for (uint64_t i = 0; i < 8u; i++) {
        termd_v2_signal_request_t signal_req;
    ...
    const struct pacha_ipc_fd *fds)
{
    ...
    if (!pacha_service_request_header_is_v2(&header, TERMD_V2_SERVICE_ID) ||
        header.request_id == 0 ||
    ...
    if (header.op == TERMD_V2_OP_SIGNAL_REGISTER_SUPERVISOR) {
