...
const ld_path = "/lib/ld-musl-x86_64.so.1";
const linux_abi_endpoint_id: u64 = 0x90;
const linux_abi_config_endpoint_id: u64 = 0x91;
const linux_abi_bootstrap_magic: u64 = 0x4C41424943464731;
const linux_abi_bootstrap_version: u64 = 1;
const linux_abi_bootstrap_ready: u64 = 0x4C414249524459;
const linux_abi_trap_request_page_va: u64 = 0x2A000000;

const max_small_file_bytes: usize = 128 * 1024;
const max_loader_file_bytes: usize = 768 * 1024;
const stack_extension_pages: u64 = 4;

var smoke_app_image: [max_small_file_bytes]u8 align(4096) = undefined;
var ld_image: [max_loader_file_bytes]u8 align(4096) = undefined;
var exec_loader_image: [max_small_file_bytes]u8 align(4096) = undefined;
var linux_abi_server_image: [max_small_file_bytes]u8 align(4096) = undefined;
var linux_abi_registry_page: [4096]u8 align(4096) = [_]u8{0} ** 4096;
var linux_abi_config_page: LinuxAbiBootstrapConfig align(4096) = .{};
...
const LinuxAbiBootstrapConfig = extern struct {
    magic: u64 = linux_abi_bootstrap_magic,
    version: u64 = linux_abi_bootstrap_version,
    exec_loader_vm_token: u64 = 0,
    abi_trap_request_page_va: u64 = linux_abi_trap_request_page_va,
    status: u64 = 0,
};
...
fn getProcessStatus(process_slot: u64) process_abi.ProcessStatusKind {
    const raw = asm volatile (
        ...
        : .{ .rcx = true, .rdx = true, .r8 = true, .r9 = true, .r10 = true, .r11 = true, .memory = true });
    return process_abi.decodeProcessStatusKind(raw);
}
...
fn appendExecArg(cfg: *exec_loader_bootstrap_abi.Config, cursor: *u16, value: []const u8) ?ArgRef {
    if (value.len == 0 or value.len > 0xffff) return null;
    const off: usize = @intCast(cursor.*);
    if (value.len > cfg.arg_data.len - off) return null;
    @memcpy(cfg.arg_data[off .. off + value.len], value);
    cursor.* += @intCast(value.len);
    cfg.arg_data_bytes = cursor.*;
    return .{ .offset = @intCast(off), .byte_len = @intCast(value.len) };
}

fn configureExecArgs(
    cfg: *exec_loader_bootstrap_abi.Config,
    execfn: []const u8,
    argv: []const []const u8,
    envp: []const []const u8,
) void {
    if (argv.len == 0 or argv.len > exec_loader_bootstrap_abi.max_argv) fail("ExecSmokeRunner: argv config invalid\n");
    if (envp.len > exec_loader_bootstrap_abi.max_envp) fail("ExecSmokeRunner: envp config invalid\n");

    var cursor: u16 = 0;
    const exec_ref = appendExecArg(cfg, &cursor, execfn) orelse fail("ExecSmokeRunner: execfn config overflow\n");
    cfg.execfn_offset = exec_ref.offset;
    cfg.execfn_bytes = exec_ref.byte_len;
    cfg.argv_count = @intCast(argv.len);
    cfg.envp_count = @intCast(envp.len);

    for (argv, 0..) |arg, index| {
        const arg_ref = appendExecArg(cfg, &cursor, arg) orelse fail("ExecSmokeRunner: argv config overflow\n");
        cfg.argv_offsets[index] = arg_ref.offset;
        cfg.argv_bytes[index] = arg_ref.byte_len;
    }
    for (envp, 0..) |env, index| {
        const env_ref = appendExecArg(cfg, &cursor, env) orelse fail("ExecSmokeRunner: envp config overflow\n");
        cfg.envp_offsets[index] = env_ref.offset;
        cfg.envp_bytes[index] = env_ref.byte_len;
    }
}
...
fn mapStaticScratchBuffers() void {
    mapStaticScratchBuffer(@intFromPtr(&smoke_app_image), smoke_app_image.len / user_vm.page_bytes);
    mapStaticScratchBuffer(@intFromPtr(&ld_image), ld_image.len / user_vm.page_bytes);
    mapStaticScratchBuffer(@intFromPtr(&exec_loader_image), exec_loader_image.len / user_vm.page_bytes);
    mapStaticScratchBuffer(@intFromPtr(&linux_abi_server_image), linux_abi_server_image.len / user_vm.page_bytes);
...
fn spawnExecLoader(
    ...
    const cfg: *exec_loader_bootstrap_abi.Config = @ptrCast(@alignCast(&exec_loader_config_page));
    cfg.* = .{
    ...
    configureExecArgs(cfg, execfn, argv, envp);
    ...
    const process_slot = process_abi.decodeSpawnedProcessSlot(spawned) orelse {
...
fn spawnLinuxAbiServer(exec_token: u64, abi_trap_request_page_va: u64, exec_loader_vm_token: u64) u64 {
    ...
    linux_abi_config_page = .{
        .magic = linux_abi_bootstrap_magic,
        .version = linux_abi_bootstrap_version,
        .exec_loader_vm_token = 0,
        .abi_trap_request_page_va = abi_trap_request_page_va,
        .status = 0,
    };
    linux_abi_bootstrap_table = .{};
    linux_abi_bootstrap_table.page_count = 2;
    linux_abi_bootstrap_table.cap_count = if (image_abi.decodeVmObjectToken(exec_loader_vm_token) != null) 1 else 0;
    linux_abi_bootstrap_table.page_descriptors[0] = .{
        .source_va = @intFromPtr(&linux_abi_config_page),
        .target_va = process_abi.standard_config_target_va,
        .flags = process_abi.spawn_flag_bootstrap_page_writable,
    };
    linux_abi_bootstrap_table.page_descriptors[1] = .{
        .source_va = @intFromPtr(&linux_abi_registry_page),
        .target_va = process_abi.service_registry_shadow_va,
        .flags = 0,
    };
    if (linux_abi_bootstrap_table.cap_count != 0) {
        linux_abi_bootstrap_table.cap_descriptors[0] = .{
            .source_token = exec_loader_vm_token,
            .target_token_va = process_abi.standard_config_target_va + @offsetOf(LinuxAbiBootstrapConfig, "exec_loader_vm_token"),
            .rights_bits = image_abi.vmObjectRightsToBits(.{ .read = true, .map = true }),
            .kind = .vm_object,
        };
    }
    return spawnExecWithExtendedBootstrapTable(exec_token, &linux_abi_bootstrap_table, true);
}

fn waitLinuxAbiServerReady() void {
    const cfg: *volatile LinuxAbiBootstrapConfig = @ptrCast(&linux_abi_config_page);
    var spins: u64 = 0;
    while (cfg.status != linux_abi_bootstrap_ready) : (spins += 1) {
        if (spins == 100_000_000) fail("ExecSmokeRunner: linux abi server ready timeout\n");
        _ = waitEvent(false, 1);
    }
}

fn waitProcessInactive(process_slot: u64, timeout_message: []const u8) void {
    var spins: u64 = 0;
    while (true) : (spins += 1) {
        switch (getProcessStatus(process_slot)) {
            .inactive => return,
            .active => {},
            .faulted => fail(timeout_message),
        }
        if (spins == 100_000_000) fail(timeout_message);
        _ = waitEvent(false, 1);
    }
}
...
pub export fn _start() noreturn {
    ...
    const loader_vm_token = installReadMapVmObject(exec_loader_image[0..loader_exec.file_bytes]);
    _ = userLog("ExecSmokeRunner: assets ready\n");
    const abi_trap_request_page_va = linux_abi_trap_request_page_va;
    const abi_server_spawned = spawnLinuxAbiServer(linux_abi_server_exec.token, abi_trap_request_page_va, loader_vm_token);
    _ = userLog("ExecSmokeRunner: linux abi server spawn returned\n");
    ...
    _ = userLog("ExecSmokeRunner: linux abi server slot decoded\n");
    waitLinuxAbiServerReady();
    _ = userLog("ExecSmokeRunner: linux abi server spawn ok\n");

    const exec_loader_process_slot = spawnExecLoader(
        loader_exec,
        smoke_app_path,
        &.{ smoke_app_path, "argv-smoke" },
        &.{ "PATH=/bin:/cmd", "CAPABILITYOS=1" },
        smoke_token,
        smoke.bytes,
        ld_token,
        ld.bytes,
    ...
    while (true) {
        switch (getProcessStatus(exec_loader_process_slot)) {
            .active => asm volatile ("pause"),
            .inactive => {
                waitProcessInactive(abi_server_process_slot, "ExecSmokeRunner: linux abi server failed\n");
                _ = userLog("ExecSmokeRunner: dynamic smoke done\n");
                processExit(0);
            },
            .faulted => {
                _ = userLog("ExecSmokeRunner: exec_loader faulted\n");
                processExit(1);
            },
        }
    }
