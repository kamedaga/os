#ifndef CAPABILITYOS_FAT_SERVER_FS_PROTOCOL_H
#define CAPABILITYOS_FAT_SERVER_FS_PROTOCOL_H
...
enum {
    ...
    FS_DIRENT_RECORD_BYTES = sizeof(struct fs_dirent_record),
};

_Static_assert(sizeof(struct fs_request_header) == FS_REQUEST_HEADER_BYTES, "fs request ABI size changed");
_Static_assert(sizeof(struct fs_response_header) == FS_RESPONSE_HEADER_BYTES, "fs response ABI size changed");
