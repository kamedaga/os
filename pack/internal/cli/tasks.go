...
func writeESPManifest(ctx *context, bootfsPath string) (string, error) {
	...
	seedApp, ok := ctx.workspace.App("seed2_boot")
	if !ok {
		return "", fmt.Errorf("missing seed2_boot app")
	}
	initApp := ctx.workspace.ArtifactPath(seedApp)
	for _, path := range []string{bootx64, initApp, bootfsPath} {
