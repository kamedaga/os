...
canonical="$repo_root/.artifacts/kobox2-virtio-net-production-canonical"
output="$repo_root/.artifacts/kobox2-virtio-net-production"
...
profile="$source_tree/kobox/manifest/profiles/virtio_net.json"
expected_config_digest="$(sed -n 's/.*"config_sha256": "\([0-9a-f]*\)".*/\1/p' "$profile")"
if [[ ! "$expected_config_digest" =~ ^[0-9a-f]{64}$ ]]; then
    echo 'virtio-net profile has no valid config digest' >&2
    exit 1
fi
...
if [[ "$current_config_digest" != "$expected_config_digest" ||
      ...
      "$source_tree/kobox/manifest/profiles/virtio_net.config" -nt "$canonical/.config" ]]; then
    ...
    "$source_tree/scripts/kconfig/merge_config.sh" -m -r -O "$canonical" \
        ...
        "$source_tree/kobox/manifest/profiles/virtio_net.config"
    make -C "$source_tree" O="$canonical" ARCH=x86 LLVM=-18 \
        KBUILD_KCONFIG="$kbuild_kconfig" olddefconfig
fi
if [[ "$(sha256sum "$canonical/.config" | cut -d' ' -f1)" != "$expected_config_digest" ]]; then
    echo 'virtio-net canonical config differs from its pinned profile' >&2
    exit 1
fi
make -C "$source_tree" O="$canonical" ARCH=x86 LLVM=-18 \
...
provider="$repo_root/.artifacts/kobox2-virtio-net-providers/$identity"
python3 "$source_tree/kobox/boot/build_boot_runtime.py" \
    ...
    --device-profile virtio-net --cc clang-18 --ld ld.lld-18 \
