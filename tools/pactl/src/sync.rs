...
use crate::config::{app_is_skipped, discover_apps, DiskPartition, WorkspaceConfig};
...
pub fn sync_bootfs_generated(
    ...
    let esp_manifest = generate_esp_manifest(
        workspace_root,
        workspace,
        &bootfs_image,
...
fn generate_esp_manifest(
    ...
    let shell_output = if let Some(shell_app) = apps
        .iter()
        .find(|app| app.app.id == "shell" && !app_is_skipped(workspace, app))
    {
        ...
        Some(output_path)
    } else if let Some(source) = shell_source {
        require_nonempty_file(source, "shell image", "run pactl build userland first")?;
    ...
    if let Some(shell_output) = shell_output {
        contents.push_str(&format!("/EFI/BOOT/SHELL.ELF={}\n", shell_output.display()));
    }
    contents.push_str(&format!(
        concat!("/EFI/BOOT/INITAPP.ELF={}\n", "/EFI/BOOT/BOOTFS.IMG={}\n"),
        init_image.display(),
