...
fn build_wsl_script(
    ...
    script.push_str("USB_STORAGE_DISK=\"$RUNTIME_DIR/usb-storage-test.raw\"\n");
    ...
    if options.split_windows {
        ...
        script.push_str("if [ -r /dev/tty ]; then old_stty=$(stty -g < /dev/tty 2>/dev/null || true); stty raw -echo min 1 time 0 < /dev/tty 2>/dev/null || true; fi\n");
        script.push_str("stty raw -echo min 1 time 0 < \"$pty\" 2>/dev/null || true\n");
        script.push_str("sync_winsize() {\n");
        script.push_str("  if [ ! -r /dev/tty ]; then return 0; fi\n");
        script.push_str("  size=$(stty size < /dev/tty 2>/dev/null || true)\n");
        script.push_str("  rows=${size%% *}\n");
        script.push_str("  cols=${size##* }\n");
        script.push_str("  case \"$rows:$cols\" in *[!0-9:]*|:|0:*|*:0) return 0 ;; esac\n");
        script.push_str("  stty rows \"$rows\" cols \"$cols\" < \"$pty\" 2>/dev/null || true\n");
        script.push_str("}\n");
        script.push_str("sync_winsize\n");
        script.push_str("trap sync_winsize WINCH\n");
    ...
    script.push_str("rm -f \"$USB_STORAGE_DISK\"\n");
    script.push_str("qemu-img create -f raw \"$NVME_DISK\" 64M >/dev/null\n");
    script.push_str("qemu-img create -f raw \"$USB_STORAGE_DISK\" 64M >/dev/null\n");
...
fn build_wsl_pf_check_script(
    ...
    script.push_str("  USB_STORAGE_DISK=\"$RUN_DIR/usb-storage-test.raw\"\n");
    ...
    script.push_str("  rm -f \"$RUN_DISK\" \"$NVME_DISK\" \"$USB_STORAGE_DISK\"\n");
    script
