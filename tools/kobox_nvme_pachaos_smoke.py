...
def qemu_cmd(run_dir: Path, disk: Path, nvme_disk: Path, usb_disk: Path | None) -> list[str]:
    cmd = [
        "qemu-system-x86_64",
        "-enable-kvm",
        "-cpu",
...
def main() -> int:
    ...
    parser.add_argument("--mode", choices=("nvme", "usb-storage"), default="nvme")
    ...
    proc = subprocess.Popen(
        qemu_cmd(run_dir, boot_disk, nvme_disk, usb_disk),
    ...
    try:
        ...
        if status != 0 or ok_count != args.iterations or "PAGE FAULT" in text or "Segmentation fault" in text:
            return 1
        return 0
    finally:
        if fd is not None:
