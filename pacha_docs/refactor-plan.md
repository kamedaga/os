...
- Step 1: signal / thread-exit boundary の完了。restorer の SysV alignment、process-directed signal の owner-thread 配送契約 (Sway TERM を `escalated=1` → `exit 0` へ反転)、musl `__unmapself` バイト照合の暫定対処を kernel post-switch-unmap flag へ置換。
  ...
  - **新規発覚バグ (Step 1 の前提条件)**: **fork した子プロセスで `pthread_create` が EAGAIN で失敗する** (`tests/run-lpr-qemu-fork-pthread-red.sh`)。親では成功。切り分け実測 (2026-07-14): ①同じ子で 128KiB `mmap`(PROT_NONE)も後続 `mprotect`(RW)も成功 → musl のスレッドスタック確保は無実。②`CLONE_THREAD` は LPR のスレッド経路 `lpr_linux_clone_thread` へ行く(fork の pid 枯渇 EAGAIN ではない)。③kernel `createThread` に一時計装 → enter 11 回・reject 0 回、ただし kernel_log(serial)と fixture marker(console)が別ストリームで fork 子の到達を相関できず未確定。④musl は clone 失敗を一律 EAGAIN に潰すため真の errno が隠れる(裏は EINVAL 系の可能性)。**次 leg の決定打**: musl マスクを外して真の errno を見る = LPR clone 経路に fixture stdout への一時トレース、または fixture で musl `__clone` 相当(LPR は start 関数を r9 で渡す独自 clone ABI)を自前発行して生戻り値を捕える。疑い先は LPR ガード(`syscalls.c:253-`/`62-`)か kernel `createThread`(`getUserSpace` が fork 子 principal に UserAddressSpace を引けるか)。**fork 後にスレッドを使う Linux アプリを全滅させるため、Phase M6.3 のアプリ導入 (GTK/foot) にも直撃する。次 leg で最優先**。
