...
**Phase 6 送りメモ (M5 中に修正しないと決めた構造課題)**: fd-ops vtable 統一 (設計ドラフト pacha_docs/fd-ops-design.md、着手時に M5 適用後のコードへ照らして見直す) / generic transferable-FD 契約・A→B→C 再転送 / native fd 窓 (fd<16) の形式化 / opcode・ABI 整理 / netd backend 参照カウント一般化とpathname owner lease / inherited filed VFS handleのsession owner lease (SIGKILL反復でactive handleが約2/周残るためmusl client側ownershipも含めて設計) / file VMO cacheの設定可能なworking-set budgetとDMA packing/fragmentation対策 / fake PRIMEのdma-buf implicit/explicit fence transport (完成後にM5.6Rの`LP_NUM_THREADS=0`を外してthreaded llvmpipeへ戻す) / `/dev/shm` tmpfs / native channelのnormal close・direct recv・normal exit・execを含むclose/wake semantics統一 (M5.6Rはpersistent poll HANGUPとSIGKILL wakeのみ) / Mesa shader-cache書き込みで再現するext4 multiply-claimed block・directory corruption。旧 Phase M6 の M6.0 SMP 以降は Phase 6 内で本整理の後段に繰り下げ。
