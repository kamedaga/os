...
**Phase M5 完了宣言**: M5.8Rの自動受け入れとフル回帰がgreenのため、Phase M5を完了とする。ただしTERM/KILL周のfiled handle残存は既知固定シグネチャ`+4/周`としてのみ許容し、根治はPhase 6のtransfer ownershipへ送る。QEMU windowでの実表示手動確認は自動スモークの代替にせず、オーケストレーター/ユーザー側で実施する。
