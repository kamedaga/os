...
- Step 1: signal / thread-exit boundary の完了。restorer の SysV alignment、process-directed signal の owner-thread 配送契約 (Sway TERM を `escalated=1` → `exit 0` へ反転)、musl `__unmapself` バイト照合の暫定対処を kernel post-switch-unmap flag へ置換。
  ...
  - **未解決の別件 flake**: sway-endurance の normal 終了で `swaymsg` が Sway IPC unix socket に接続できない**間欠 flake** (~1/7、fork 修正とは無関係と複数サンプルで確定、fork 修正前から存在の可能性)。battery-v2 の retry で吸収される。調査方針: serial の失敗 `NETD_OP_CONNECT` status と直前の `unix_orphan_reap` を相関。
  - **signal-owner 配送修正済み (2026-07-14, `8b97a58`)**: §9.4 item 1 の核心。kernel `deliverSignal` が「最初に block したスレッド」へ process 宛シグナルを配送していた欠陥を、`signal_owner` フラグ (REGISTER = main/event-loop が立てる) による owner 優先配送へ修正。fork 子は `runtime_registered` 継承で REGISTER をスキップし owner 不在になるため、fork-child hook で同フラグをリセットして子 main が再登録するようにした (これが無いと owner-preference が first-blocked のレースへ退化し間欠的に worker を選ぶ — battery が捕捉)。signal-owner smoke が HANDLER=WORKER→OWNER で 4/4 決定的 green。host 6/6 + battery 26/26。
  - **Step 1 の未達 (証拠が伴わず見送り)**: ① **TERM oracle 反転せず** — owner 配送だけでは Sway TERM は依然 escalate (`escalated=1`)。設計の「owner 配送→graceful」前提は不完全で、Sway teardown の真のブロッカーはスレッド選択とは別 (**次の調査対象**)。現 `escalated=1` が現実を正しく記述するため反転しない。② **§9.4 item 6 (active-stack post-switch-unmap) 後回し** — red fixture が測定点前にハングし「正しい理由の red」が作れず、既存の musl バイト照合暫定対処が実ワークロードで機能中 (red を `.temp-docs/wip-step1-reds/` へ退避)。③ **sub-op renumber スキップ** (機能無関係・純リスク)。
  - **運用メモ**: signal/thread/asm 領域は codex が OpenAI content filter に繰り返し阻まれるため、この Step の kernel 修正はオーケストレーター (Claude) が直接実装・検証した。
  - kernel 申請 dossier: `.temp-docs/step1-kernel-request.md` (item 6 は red 修復後に再提出)
