...
旧`smp-wip`は一括mergeしない。現在の`main`へ必要なboot plumbingを小さい変更として移植する。

### P2.1 1 CPU上でinvariantを固定

APを動かす前に、次を1 CPUで検証可能にする。

- allocator free list整合性
- page retain / release
- thread state遷移
- process teardown
- COW
...
race testを追加しても1 CPUで意味があるownership違反を先に除く。

### P2.2 AP boot plumbing

- Limine RSDP / memory map
- MADT解析
...
最初はAPをonlineにするが、user threadを割り当てずparkする。

### P2.3 allocator同期

- global free listへの全accessを正式なAPIへ通す。
- syscall、page fault、COW、mmap、munmap、exec、teardownを監査する。
- IRQ contextでlockを取得する場合の規則を明示する。
- lock orderを文書化する。
- page数保存と二重freeをunit testで検証する。

初期段階ではglobal lockを許容する。性能最適化は正しさ成立後に行う。

### P2.4 thread stateとCPU ownership
...
同じthreadを2 CPUが同時実行できないこと、remote CPU上のthreadをlocal CPUが直接teardownしないことを保証する。generationまたは同等の方法で古いwake / stop ackを区別する。

### P2.5 remote wake / stop / kill
...
timer tickが来るまでwakeしない構造にしない。IPI handlerとscheduler lockの順序を固定し、lost wakeupと二重enqueueをtestする。

### P2.6 timer preemption

AP timer interruptからpreemptするために、次の所有権と保存範囲を明示する。
...
user contextとkernel contextを混同しない。preemption不可区間を最小化し、解除漏れを観測可能にする。

### P2.7 TLB shootdown

address spaceごとにactive CPU maskを持つ。
...
上記で必要なCPUへshootdownを送り、ackを待つ。古いmappingを使うCPUが残ったままpageを再利用しない。

### P2.8 kernel全体の共有状態監査
...
- IRQ routing
- `_kobox`との共有境界
- debug / metric counter

module-level mutable stateごとにowner CPU、lock、atomic、read-onlyのいずれかを明示する。
...
失敗時にCPU数を戻して成功扱いにしない。各段階のraceを修正してから次へ進む。
