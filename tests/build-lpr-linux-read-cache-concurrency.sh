...
out="$PWD/.artifacts/tests/lpr-linux-read-cache-concurrency"
mkdir -p "$out"
clang -target x86_64-linux-musl --sysroot="$sysroot" -std=c11 -O2 -Wall -Wextra -Werror \
    -fPIC -c tests/lpr_linux_read_cache_concurrency.c -o "$out/test.o"
clang -target x86_64-linux-musl --sysroot="$sysroot" -nostdlib \
    ...
    "$sysroot/usr/lib/crtn.o" -o "$out/read-cache"
