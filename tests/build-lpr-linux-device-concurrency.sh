...
out="$repo_root/.artifacts/tests/lpr-linux-device-concurrency"
mkdir -p "$out"
/usr/bin/clang -target x86_64-linux-musl --sysroot="$sysroot" \
  -std=c11 -O2 -Wall -Wextra -Werror -fPIC \
  -c "$repo_root/tests/lpr_linux_device_concurrency.c" -o "$out/test.o"
/usr/bin/clang -target x86_64-linux-musl --sysroot="$sysroot" -nostdlib \
  ...
  "$sysroot/usr/lib/crtn.o" -o "$out/device-concurrency"
