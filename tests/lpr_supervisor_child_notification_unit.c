...
int main(void)
{
    ...
    lprs_notify_exited_child(&processes[1], 37);
    ...
    lprs_notify_exited_child(&processes[1], 73);
    CHECK(processes[1].exit_status == 37);
    CHECK(g_syscall_count == 1);

    CHECK(lprs_find_exited_child(
        &processes[0], -1, &selected_index, &exit_code, &match_count) == 0);
    ...
    g_syscall_result = -7;
    lprs_notify_exited_child(&processes[2], 91);
    ...
    lprs_notify_exited_child(&processes[2], 92);
    CHECK(processes[2].exit_status == 91);
    CHECK(g_syscall_count == 2);
