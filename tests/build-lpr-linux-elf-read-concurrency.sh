...
out="$repo_root/.artifacts/tests/lpr-linux-elf-read-concurrency"
mkdir -p "$out"
[[ -f "$sysroot/usr/lib/Scrt1.o" && -f "$runtime_libc" ]] || {
  echo "Build the pinned Alpine clang sysroot and runtime libc first." >&2
  exit 1
}
/usr/bin/clang -target x86_64-linux-musl --sysroot="$sysroot" \
  -std=c11 -O2 -Wall -Wextra -Werror -fPIC \
  -c "$repo_root/tests/lpr_linux_elf_read_concurrency.c" -o "$out/test.o"
/usr/bin/clang -target x86_64-linux-musl --sysroot="$sysroot" -nostdlib \
  ...
  "$sysroot/usr/lib/crtn.o" -o "$out/elf-read-test"
printf '%s\n' "$out/elf-read-test"
