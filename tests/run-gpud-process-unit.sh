...
out="$repo_root/.artifacts/tests/gpud-process-unit/${CC:-cc}"
mkdir -p "$out"
sources=("$repo_root/tests/gpud_process_unit.c"
...
"${CC:-cc}" -std=c11 -O1 -g -Wall -Wextra -Werror -Wpedantic \
  ...
  "${sources[@]}" -o "$out/gpud-process-unit"
"$out/gpud-process-unit" | tee "$out/result.log"
sha256sum "${sources[@]}" "$repo_root/userland/gpud/process.h" \
  "$repo_root/userland/gpud/process_native.h" "$repo_root/userland/kobox2_adapter/ipc.h" \
  "$repo_root/tests/run-gpud-process-unit.sh" >"$out/inputs.sha256"
