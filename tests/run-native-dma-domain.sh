...
# Bootloader image only; no rootfs disk is copied or attached.
set -euo pipefail
repo_root="$(cd "$(dirname "$0")/.." && pwd)"
out="$repo_root/.artifacts/tests/native-dma-domain"
mkdir -p "$out"
"${CAPOS_FREESTANDING_CC:-clang}" -target x86_64-linux-gnu -fuse-ld=lld \
  ...
  "$repo_root/tests/native_dma_domain.c" \
  "$repo_root/userland/kobox2_adapter/device_dma.c" \
  "$repo_root/userland/libpacha/src/syscall.c" \
  -Wl,-e,_start,--image-base=0x400000,-z,noexecstack -o "$out/native_dma_domain.elf"
...
mcopy -o -i "$out/boot.img@@2097152" "$out/native_dma_domain.elf" ::/INITAPP.ELF
mcopy -o -i "$out/boot.img@@2097152" "$out/bootfs.img" ::/BOOTFS.IMG
sha256sum "$repo_root/kernel/zig-out/bin/limine/pacha-kernel.elf" \
  "$out/native_dma_domain.elf" >"$out/inputs.sha256"
sha256sum "$repo_root/tests/native_dma_domain.c" "$repo_root/tests/native_device_contract.c" \
  "$repo_root/tests/run-native-dma-domain.sh" \
  "$repo_root/userland/kobox2_adapter/device_dma.c" \
  "$repo_root/userland/kobox2_adapter/device_dma.h" >"$out/sources.sha256"
: >"$out/serial.log"
qemu-system-x86_64 -machine q35 -cpu host -enable-kvm -m 2G -smp 2 \
  -trace 'enable=virtio_rng_*' -trace enable=vtd_dmar_fault \
...
while kill -0 "$qemu_pid" 2>/dev/null && [[ "$SECONDS" -lt "$deadline" ]]; do
  if rg -q 'NATIVE_DMA_DOMAIN=PASS|NATIVE_DEVICE_CONTRACT=FAIL|ACTION=terminate|PANIC|panic|init ELF load failed' "$out/serial.log"; then break; fi
  sleep 0.2
done
stop_qemu
trap - EXIT
if ! rg -q 'NATIVE_DMA_DOMAIN=PASS' "$out/serial.log"; then tail -100 "$out/serial.log"; exit 1; fi
if rg -n 'NATIVE_DEVICE_CONTRACT=FAIL|PANIC|panic|ACTION=terminate' "$out/serial.log"; then exit 1; fi
if [[ $(rg -c '^vtd_dmar_fault ' "$out/qemu.log" || true) -lt 4 ]]; then
  printf 'Missing evidence of actual blocked IOMMU accesses\n' >&2
  exit 1
fi
printf 'Native DMA domain passed: %s\n' "$out/serial.log"
