...
# Boot-only IRQ test; no rootfs disk is copied or attached.
set -euo pipefail
repo_root="$(cd "$(dirname "$0")/.." && pwd)"
out="$repo_root/.artifacts/tests/native-irq-retire"
mkdir -p "$out"
"${CAPOS_FREESTANDING_CC:-clang}" -target x86_64-linux-gnu -fuse-ld=lld \
  ...
  "$repo_root/tests/native_irq_retire.c" "$repo_root/userland/libpacha/src/syscall.c" \
  -Wl,-e,_start,--image-base=0x400000,-z,noexecstack -o "$out/native_irq_retire.elf"
...
mcopy -o -i "$out/boot.img@@2097152" "$out/native_irq_retire.elf" ::/INITAPP.ELF
mcopy -o -i "$out/boot.img@@2097152" "$out/bootfs.img" ::/BOOTFS.IMG
sha256sum "$repo_root/kernel/zig-out/bin/limine/pacha-kernel.elf" \
  "$out/native_irq_retire.elf" >"$out/inputs.sha256"
sha256sum "$repo_root/tests/native_irq_retire.c" "$repo_root/tests/native_device_contract.c" \
  "$repo_root/tests/run-native-irq-retire.sh" >"$out/sources.sha256"
: >"$out/serial.log"
...
while kill -0 "$qemu_pid" 2>/dev/null && [[ "$SECONDS" -lt "$deadline" ]]; do
  if rg -q 'NATIVE_IRQ_RETIRE=PASS|NATIVE_DEVICE_CONTRACT=FAIL|ACTION=terminate|PANIC|panic|init ELF load failed' "$out/serial.log"; then break; fi
  sleep 0.2
done
stop_qemu
trap - EXIT
if ! rg -q 'NATIVE_IRQ_RETIRE=PASS' "$out/serial.log"; then tail -100 "$out/serial.log"; exit 1; fi
if rg -n 'NATIVE_DEVICE_CONTRACT=FAIL|PANIC|panic|ACTION=terminate' "$out/serial.log"; then exit 1; fi
printf 'Native IRQ retirement passed: %s\n' "$out/serial.log"
