...
out="$PWD/.artifacts/tests/lpr-linux-socket-type"
mkdir -p "$out"
clang -std=c11 -O2 -Wall -Wextra -Werror tests/lpr_linux_socket_type.c -o "$out/host"
"$out/host"
clang -target x86_64-linux-musl --sysroot="$sysroot" -std=c11 -O2 -Wall -Wextra -Werror \
    -fPIC -c tests/lpr_linux_socket_type.c -o "$out/test.o"
clang -target x86_64-linux-musl --sysroot="$sysroot" -nostdlib \
    ...
    "$sysroot/usr/lib/crtn.o" -o "$out/socket-type"
