...
out="$PWD/.artifacts/tests/lpr-linux-rights-concurrency"
mkdir -p "$out"
clang -target x86_64-linux-musl --sysroot="$sysroot" -std=c11 -O2 -Wall -Wextra -Werror \
    -fPIC -c tests/lpr_linux_rights_concurrency.c -o "$out/test.o"
clang -target x86_64-linux-musl --sysroot="$sysroot" -nostdlib \
    ...
    "$sysroot/usr/lib/crtn.o" -o "$out/rights-concurrency"
