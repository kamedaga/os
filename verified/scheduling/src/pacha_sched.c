#include "pacha_sched.h"

static pacha_sched_rc map_eevdf_rc(pacha_eevdf_rc rc) {
  switch (rc) {
  case PACHA_EEVDF_OK:
    return PACHA_SCHED_OK;
  case PACHA_EEVDF_ERR_INVALID:
    return PACHA_SCHED_ERR_INVALID;
  case PACHA_EEVDF_ERR_FULL:
    return PACHA_SCHED_ERR_FULL;
  case PACHA_EEVDF_ERR_OVERFLOW:
    return PACHA_SCHED_ERR_OVERFLOW;
  case PACHA_EEVDF_ERR_STATE:
    return PACHA_SCHED_ERR_STATE;
  }
  return PACHA_SCHED_ERR_INVALID;
}

void pacha_sched_no_decision(pacha_sched_decision *out) {
  out->kind = PACHA_SCHED_DECISION_NONE;
  out->cpu_id = PACHA_SCHED_NO_CPU;
  out->thread_id = PACHA_EEVDF_NO_THREAD_ID;
  out->generation = 0;
  return;
}

static void run_thread_decision(
    size_t cpu_id,
    const pacha_eevdf_entity *entity,
    pacha_sched_decision *out) {
  out->kind = PACHA_SCHED_DECISION_RUN_THREAD;
  out->cpu_id = cpu_id;
  out->thread_id = entity->thread_id;
  out->generation = entity->generation;
  return;
}

static void idle_decision(size_t cpu_id, pacha_sched_decision *out) {
  out->kind = PACHA_SCHED_DECISION_IDLE;
  out->cpu_id = cpu_id;
  out->thread_id = PACHA_EEVDF_NO_THREAD_ID;
  out->generation = 0;
  return;
}

static int valid_cpu(const pacha_sched_state *sched, size_t cpu_id) {
  return cpu_id < sched->cpu_count;
}

static const pacha_sched_cpu *sched_cpu_const_ptr(
    const pacha_sched_state *sched,
    size_t cpu_id) {
  return sched->cpus + cpu_id;
}

static pacha_sched_cpu *sched_cpu_ptr(
    pacha_sched_state *sched,
    size_t cpu_id) {
  return sched->cpus + cpu_id;
}

static int sched_cpu_has_current(const pacha_sched_cpu *cpu) {
  return cpu->has_current;
}

static int cpu_has_current(const pacha_sched_state *sched, size_t cpu_id) {
  int result = 0;
  if (valid_cpu(sched, cpu_id)) {
    result = sched_cpu_has_current(sched_cpu_const_ptr(sched, cpu_id));
  }
  return result;
}

static void clear_cpu_current(pacha_sched_cpu *cpu) {
  cpu->has_current = 0;
  cpu->current_thread_id = PACHA_EEVDF_NO_THREAD_ID;
  return;
}

static void set_cpu_current(
    pacha_sched_cpu *cpu,
    int64_t thread_id) {
  cpu->has_current = 1;
  cpu->current_thread_id = thread_id;
  return;
}

static void clear_current_if_matches(
    pacha_sched_state *sched,
    int64_t thread_id) {
  for (size_t cpu = 0; cpu < sched->cpu_count; ++cpu) {
    pacha_sched_cpu *slot = sched_cpu_ptr(sched, cpu);
    if (slot->has_current && slot->current_thread_id == thread_id) {
      clear_cpu_current(slot);
    }
  }
}

void pacha_sched_empty_state(size_t cpu_count, pacha_sched_state *out) {
  pacha_eevdf_empty_runqueue(&out->runqueue);
  out->cpu_count = cpu_count;
  if (out->cpu_count > PACHA_SCHED_MAX_CPUS) {
    out->cpu_count = PACHA_SCHED_MAX_CPUS;
  }
  for (size_t i = 0; i < PACHA_SCHED_MAX_CPUS; ++i) {
    out->cpus[i].has_current = 0;
    out->cpus[i].current_thread_id = PACHA_EEVDF_NO_THREAD_ID;
  }
  return;
}

pacha_sched_rc pacha_sched_add_thread(
    pacha_sched_state *sched,
    int64_t thread_id,
    int64_t generation,
    int64_t weight,
    int64_t slice_ns,
    pacha_sched_decision *decision_out,
    pacha_eevdf_runqueue *scratch) {
  pacha_sched_no_decision(decision_out);
  pacha_eevdf_rc rc = pacha_eevdf_add(
      &sched->runqueue, thread_id, generation, weight, slice_ns, scratch);
  if (rc != PACHA_EEVDF_OK) {
    return map_eevdf_rc(rc);
  }
  pacha_eevdf_copy_runqueue(scratch, &sched->runqueue);
  return PACHA_SCHED_OK;
}

pacha_sched_rc pacha_sched_wake_thread(
    pacha_sched_state *sched,
    int64_t thread_id,
    pacha_sched_decision *decision_out,
    pacha_eevdf_runqueue *scratch) {
  pacha_sched_no_decision(decision_out);
  pacha_eevdf_rc rc = pacha_eevdf_wake(&sched->runqueue, thread_id, scratch);
  if (rc != PACHA_EEVDF_OK) {
    return map_eevdf_rc(rc);
  }
  pacha_eevdf_copy_runqueue(scratch, &sched->runqueue);
  return PACHA_SCHED_OK;
}

pacha_sched_rc pacha_sched_block_thread(
    pacha_sched_state *sched,
    int64_t thread_id,
    pacha_sched_decision *decision_out,
    pacha_eevdf_runqueue *scratch) {
  pacha_sched_no_decision(decision_out);
  pacha_eevdf_rc rc = pacha_eevdf_block(&sched->runqueue, thread_id, scratch);
  if (rc != PACHA_EEVDF_OK) {
    return map_eevdf_rc(rc);
  }
  pacha_eevdf_copy_runqueue(scratch, &sched->runqueue);
  clear_current_if_matches(sched, thread_id);
  return PACHA_SCHED_OK;
}

pacha_sched_rc pacha_sched_exit_thread(
    pacha_sched_state *sched,
    int64_t thread_id,
    pacha_sched_decision *decision_out,
    pacha_eevdf_runqueue *scratch) {
  pacha_sched_no_decision(decision_out);
  pacha_eevdf_rc rc = pacha_eevdf_exit(&sched->runqueue, thread_id, scratch);
  if (rc != PACHA_EEVDF_OK) {
    return map_eevdf_rc(rc);
  }
  pacha_eevdf_copy_runqueue(scratch, &sched->runqueue);
  clear_current_if_matches(sched, thread_id);
  return PACHA_SCHED_OK;
}

static pacha_sched_rc on_timer_valid(
    pacha_sched_state *sched,
    size_t cpu_id,
    int64_t runtime_ns,
    pacha_eevdf_runqueue *scratch) {
  if (sched->cpus[cpu_id].has_current) {
    int64_t thread_id = sched->cpus[cpu_id].current_thread_id;
    pacha_eevdf_rc rc =
        pacha_eevdf_charge(&sched->runqueue, thread_id, runtime_ns, scratch);
    if (rc != PACHA_EEVDF_OK) {
      return map_eevdf_rc(rc);
    }
    pacha_eevdf_copy_runqueue(scratch, &sched->runqueue);
    return PACHA_SCHED_OK;
  }
  return PACHA_SCHED_OK;
}

pacha_sched_rc pacha_sched_on_timer(
    pacha_sched_state *sched,
    size_t cpu_id,
    int64_t runtime_ns,
    pacha_sched_decision *decision_out,
    pacha_eevdf_runqueue *scratch) {
  pacha_sched_no_decision(decision_out);
  if (cpu_id < sched->cpu_count) {
    return on_timer_valid(sched, cpu_id, runtime_ns, scratch);
  }
  return PACHA_SCHED_ERR_INVALID;
}

pacha_sched_rc pacha_sched_pick(
  pacha_sched_state *sched,
  size_t cpu_id,
  pacha_sched_decision *decision_out,
  pacha_eevdf_pick_result *pick_scratch,
  pacha_eevdf_runqueue *scratch) {
  pacha_sched_no_decision(decision_out);
  if (!valid_cpu(sched, cpu_id)) {
    return PACHA_SCHED_ERR_INVALID;
  }
  if (cpu_has_current(sched, cpu_id)) {
    return PACHA_SCHED_ERR_STATE;
  }

  pacha_eevdf_rc pick_rc = pacha_eevdf_pick(&sched->runqueue, pick_scratch);
  if (pick_rc != PACHA_EEVDF_OK) {
    return map_eevdf_rc(pick_rc);
  }
  pacha_eevdf_copy_runqueue(&pick_scratch->rq, &sched->runqueue);
  if (!pick_scratch->has_entity) {
    idle_decision(cpu_id, decision_out);
    return PACHA_SCHED_OK;
  }

  pacha_eevdf_rc mark =
      pacha_eevdf_mark_running(
          &sched->runqueue,
          pick_scratch->entity.thread_id,
          scratch);
  if (mark != PACHA_EEVDF_OK) {
    pacha_sched_no_decision(decision_out);
    return map_eevdf_rc(mark);
  }
  pacha_eevdf_copy_runqueue(scratch, &sched->runqueue);
  set_cpu_current(&sched->cpus[cpu_id], pick_scratch->entity.thread_id);
  run_thread_decision(cpu_id, &pick_scratch->entity, decision_out);
  return PACHA_SCHED_OK;
}

static pacha_sched_rc finish_current_valid(
    pacha_sched_state *sched,
    size_t cpu_id,
    pacha_eevdf_runqueue *scratch) {
  pacha_sched_cpu *cpu = sched_cpu_ptr(sched, cpu_id);
  if (sched_cpu_has_current(cpu)) {
    int64_t thread_id = cpu->current_thread_id;
    pacha_eevdf_rc rc =
        pacha_eevdf_requeue_running(&sched->runqueue, thread_id, scratch);
    if (rc != PACHA_EEVDF_OK) {
      return map_eevdf_rc(rc);
    }
    pacha_eevdf_copy_runqueue(scratch, &sched->runqueue);
    clear_cpu_current(cpu);
    return PACHA_SCHED_OK;
  }
  return PACHA_SCHED_OK;
}

pacha_sched_rc pacha_sched_finish_current(
    pacha_sched_state *sched,
    size_t cpu_id,
    pacha_sched_decision *decision_out,
    pacha_eevdf_runqueue *scratch) {
  pacha_sched_no_decision(decision_out);
  if (cpu_id < sched->cpu_count) {
    return finish_current_valid(sched, cpu_id, scratch);
  }
  return PACHA_SCHED_ERR_INVALID;
}
