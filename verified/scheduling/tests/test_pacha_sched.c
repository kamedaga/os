...
static void test_readd_exited_thread_id_reuses_slot(void) {
  pacha_sched_state sched;
  pacha_sched_decision decision;
  pacha_eevdf_runqueue scratch;
  pacha_sched_empty_state(1, &sched);
  ...
  assert(sched.runqueue.runnable_count == 1);
  assert(sched.runqueue.entities[0].thread_id == 1);
  assert(sched.runqueue.entities[0].generation == 8);
}

static void test_stale_current_generation_is_cleared_after_exit(void) {
  pacha_sched_state sched;
  pacha_sched_decision decision;
  pacha_eevdf_runqueue scratch;
  pacha_eevdf_pick_result pick_scratch;
  ...
  assert(pacha_sched_on_timer(&sched, 0, 1000, &decision, &scratch) ==
      PACHA_SCHED_OK);
  assert(!sched.cpus[0].has_current);
  assert(sched.runqueue.entity_count == 0);
...
int main(void) {
  ...
  test_readd_exited_thread_id_reuses_slot();
  test_stale_current_generation_is_cleared_after_exit();
  return 0;
}
